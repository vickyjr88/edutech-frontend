      ...
      <div className="flex items-center justify-between mb-2 overflow-x-auto">
        <div className="w-full flex items-center justify-between">
          {Array.from({ length: totalSteps }).map((_, index) => {
            // A step is completed if it's explicitly marked as complete in the status array
            const isCompleted = completionStatus[index] === true;
              ...
              >
                <div className={`flex items-center ${index !== 0 && index !== totalSteps - 1 ? 'w-full' : ''}`}>
                  {index !== 0 && (
                    <div 
                      className={`h-1 w-full max-w-12 sm:max-w-16 md:max-w-24 ${
                        completionStatus[index-1] === true ? 'bg-green-500' : 'bg-gray-200'
                      }`}
                    />
                  )}
                  <div
                  ...
                  {index !== totalSteps - 1 && (
                    <div 
                      className={`h-1 w-full max-w-12 sm:max-w-16 md:max-w-24 ${
                        completionStatus[index] === true ? 'bg-green-500' : 'bg-gray-200'
                      }`}
                    />
                  )}
                </div>
