...
const CheckoutFlow = ({ onClose, enrollment }: CheckoutFlowProps) => {
    ...
    const { data: classDataResponse } = useClassById(enrollment.course.id);
    ...
    const courseData = {
        ...
        schedule: `${formatDate(enrollment?.nextClass?.date, 'EEEE')}, ${enrollment?.nextClass?.time}`,
        startDate: formatDate(classData.cohorts[0]?.startDate, 'MMMM d, yyyy'),
        enrollmentDeadline: formatDate(classData.cohorts[0]?.endDate, 'MMMM d, yyyy'),
        spotsRemaining: enrollment?.participants?.maximum - enrollment?.participants?.current,
        duration: "12 weeks",
