...
interface BoyaPaymentFormFixedProps {
  amount: number;
  currency?: string;
...
const BoyaPaymentFormFixed: React.FC<BoyaPaymentFormFixedProps> = ({
  ...
  // Debug logging
  useEffect(() => {
    console.log('BoyaPaymentFormFixed initialized with BT:', !!bt);
  }, [bt]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Step 1: Creating token with Basis Theory...');

      // Step 1: Create a token using Basis Theory elements
      ...
      console.log('Step 1 Complete: Token created successfully');
      console.log('Step 2: Processing payment with Boya...');

      // Step 2: Process payment with Boya using the fingerprint
      const paymentResponse = await paymentService.processPayment({
        fingerprint: token.fingerprint || token.id, // Try fingerprint first, fallback to id
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button
            type="submit"
            className="w-full"
            disabled={isLoading || !cardholder.trim() || !bt}
          >
            {isLoading ? (
              <>
...
interface BoyaPaymentWrapperFixedProps extends BoyaPaymentFormFixedProps {
  apiKey: string;
}

const BoyaPaymentWrapperFixed: React.FC<BoyaPaymentWrapperFixedProps> = ({
  apiKey,
  ...props
}) => {
  if (!apiKey) {
...
export default BoyaPaymentWrapperFixed;
export { BoyaPaymentFormFixed };
export type { BoyaPaymentFormFixedProps };